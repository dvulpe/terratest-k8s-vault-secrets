...
func TestVaultClient(t *testing.T) {
	...
	k8s.WaitUntilServiceAvailable(t, options, "vault", 10, 1*time.Second)
	k8s.WaitUntilNumPodsCreated(t, options, metav1.ListOptions{LabelSelector: "app = vault"}, 1, 10, 1*time.Second)
	pods := k8s.ListPods(t, options, metav1.ListOptions{LabelSelector: "app = vault"})
	if len(pods) != 1 {
		t.Fatalf("expected 1 pod, got: %v", len(pods))
	}
	k8s.WaitUntilPodAvailable(t, options, pods[0].Name, 10, 1*time.Second)
	...
	k8s.WaitUntilNumPodsCreated(t, options, metav1.ListOptions{LabelSelector: "app = workload"}, 1, 100, 1*time.Second)
	appPods := k8s.ListPods(t, options, metav1.ListOptions{LabelSelector: "app = workload"})
	if len(appPods) != 1 {
		t.Fatalf("expected 1 pod, got: %v", len(pods))
	}
	k8s.WaitUntilPodAvailable(t, options, appPods[0].Name, 10, 1*time.Second)
}
